...
### [通知](CCTools/) (notify + CCTools)

Claude Code 任务完成、等待输入、API 错误时发送桌面通知。

`notify/notify-cc-tools.sh` 作为 hook 适配层，将事件通过 HTTP 转发到 [CCTools](CCTools/) 菜单栏 app，由 app 显示浮窗通知并记录通知历史。另有 `notify/notify-osc.sh` 通过 OSC 777 终端转义序列通知，适用于 Warp/Ghostty 等终端。
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/AI/so-cc-tools/notify/notify-cc-tools.sh"
          }
        ]
      }
    ],
...
脚本读取当前配置、合并回 `statusLine` 与 `hooks`（通知走 `notify-cc-tools.sh` → CCTools app），再原子写回。特性：
...
- bash、jq（必需）
- [CCTools](CCTools/) macOS app（推荐，需要 macOS 14.0+）
